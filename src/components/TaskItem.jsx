...
export default function TaskItem({ task, customFactors = [], onComplete, onDelete, onEdit }) {
  const score = computePriorityScore(task, customFactors);
      ...
      <div className="task-content">
        ...
        <div className="task-meta">
          ...
          {customFactors.map(factor => {
            ...
            if (!val) return null;
            return (
              <span key={factor.id} className="badge badge-custom" title={factor.name}>
                {factor.name}: {val}
              </span>
            );
          })}
